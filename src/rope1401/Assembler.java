...
class Assembler
{
	...
	static String getTimeID() throws Exception
	{
	...
	static void buildTape() throws IOException
	{
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(AssemblerOptions.tapePath))) 
		{
			String line;
			while ((line = reader.readLine()) != null) 
			{
				int len = line.length();
				String id = line.substring(len - 7, len - 3);
				if(id.compareTo("    ") == 0)
				{
					line = line.substring(0, len - 7) + timeID + " ~}";
				}
		...
		if(lines.size() > 0)
		{
			File oldFile = new File(AssemblerOptions.tapePath);
			oldFile.delete();
				
			try (FileWriter writer = new FileWriter(AssemblerOptions.tapePath)) 
			{
	...
	static void buildDeck() throws IOException
	{
		...
		List<String> lines = new ArrayList<>();		
		try (BufferedReader reader = new BufferedReader(new FileReader(AssemblerOptions.objectPath))) 
		{
			String line;
			while ((line = reader.readLine()) != null) 
			{
				if(line.length() == 75)
				{
					lines.add(line.concat(timeID));
				}
				else
				{
					System.out.println("Invalid Deck file format!");
		...
		if(lines.size() > 0)
		{
			File oldFile = new File(AssemblerOptions.objectPath);
			oldFile.delete();
				
			try (FileWriter writer = new FileWriter(AssemblerOptions.objectPath)) 
			{
				for(String line : lines)
				{
					writer.write(line + "\n");
