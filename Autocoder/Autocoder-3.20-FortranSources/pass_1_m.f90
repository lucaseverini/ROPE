module PASS_1_M
  ...
  use IO_UNITS, only: U_SCRATCH, U_SCR2
...
contains
  ...
  subroutine PASS_1 ( IOSTAT, NeedPass2 )
    ...
    do
      ...
      read ( u_scr2, 10, iostat=iostat ) why, errCode, line
10    format ( a5, a1, a )
      if ( iostat < 0 ) then  ! End of file
        ...
        call process_ltorg ( p, .true., 'Pass 1' )
        exit
...
!       & call do_error ( 'First card is not a JOB card' )
      ...
      if ( line(6:6) == '*' &    ! Comment line
        ...
        write ( u_scratch, 200 ) why, line, 0,     0, 0,     errCode,   0
        cycle
      end if

      if ( why == 'LITS ' ) then
          !                             ixlab  P Width ErrCode Num_Operands
        write ( u_scratch, 200 ) why, line, 0, 0, 0,       ' ',  0
        call process_ltorg ( p, .true., 'Pass 1' )
        cycle
      end if
        ...
        case ( 'EQU' )
          ...
          if ( .not. error ) then
            ...
            case ( k_actual, k_asterisk )
              ...
              width = 0
            case ( k_device )
              call enter ( label, 0, 0, ixlab, dev=operands(1)%label )
            case ( k_symbolic )
              if ( operands(1)%label(6:6) == '' ) operands(1)%label(6:6) = sfx
              call enter ( operands(1)%label, ref, 0, operands(1)%addr )
...
999   write ( u_scratch, 200 ) why, line, ixLab, p_scratch, width, &
        & errCode, num_operands, operands(1:num_operands)
...
!      if ( line(16:20) == 'LTORG' ) call process_ltorg ( p, .true., 'Pass 1' )
  ...
  contains
    ...
    subroutine DCW ( The_Operand )
      ...
      case ( k_actual, k_adcon_lit, k_addr_con, k_asterisk, k_blank_con, &
        ...
        if ( the_operand%kind == k_addr_con ) then
          call enter ( the_operand%label , ref, 0, the_operand%addr )
          width = 3
        else if ( the_operand%kind == k_adcon_lit ) then
          call create_lit ( l_char_lit, width, line(pos-width-2:pos-3), &
            & the_operand%addr )
          width = 3
    ...
    subroutine END_OR_EX
      ...
      case ( t_done )
        operands(1) = operand(0,k_actual,0,'  ',' ','      ')
      case ( t_name )
        operands(1) = operand(0,k_symbolic,0,'  ',' ',line(pos:end))
        if ( operands(1)%label(6:6) == '' ) operands(1)%label(6:6) = sfx
        call enter ( operands(1)%label, ref, 0, operands(1)%addr )
      case ( t_number )
        operands(1) = operand(0,k_actual,0,'  ',' ','      ')
