               JOB  Compute pi to 100 decimal places
               CTL  6611
     ***************************************************************************
     *
     *     Compute pi to 100 decimal places using the Borwein algorithm:
     *
     *     Initialization
     *
     *         y[0] = sqrt(2) - 1
     ...
     *         pi[i]  = 1/a[i]
     *
     *     After the iterations are done, pi = pi[3].
     *
     *     To ensure accuracy of all 100 decimal places, this program
     *     calculates with 105 digits.
     *
     ***************************************************************************
               ORG  87
     ...
     *
     START     CS   332
               CS
               MCW  @***** Pi to 100 Decimal Places *****@,319
               W
               CS   332
               CS
               W
               MCW  @----- Initialization -----@,319
               W
     *
     * Compute and print sqrt(2).
     *
     ...
     *
               ...
               B    SQRT
     *
               SBR  PRADDR,SQRT2       PRADDR = @SQRT2 - dp
               ZA   DP,CNVOFF
               B    CNVRTN
               MA   CNVOFF,PRADDR
               ...
               W
     *
     * Compute and print y[0] = sqrt(2) - 1
     *
               MCW  SQRT2,YI           sqrt(2)
               S    @1@,YI-105                 - 1
               MZ   @ @,YI-105
     ...
     *
               ...
               W
     *
     * Compute and print a[0] = 6 - 4*sqrt(2)
     *
               S    AI
               MCW  @6@,AI-105        6
               S    SQRT2,AI            - sqrt(2)
     ...
     *
               ...
               W
     *
     * Start of loop.  Increment and print the iteration counter.
     *
     LOOP      A    @1@,ITERS
     *
               CS   332
     ...
     *
               ...
               W
     *
     * Compute and print yrt4 = (1 - y4)**(1/4)
     *
               S    TEMP3&105
               MCW  @1@,TEMP3          1
               S    Y4,TEMP3&105         - y4
     ...
     *
               SBR  PRADDR,YRT4        PRADDR = @YRT4 - dp
               ZA   DP,CNVOFF
               B    CNVRTN
               MA   CNVOFF,PRADDR
               ZA   DPPRT,PRDP         PRDP = dp to print
               ...
               W
     *
     * Compute and print y[i] = (1 - yrt4)/(1 + yrt4)
     *
               S    YI&107
               MCW  @1@,YI-103         1
     ...
     *
               ZA   YRT4,Y2            yrt4
               A    @1@,Y2-105              + 1 (divisor)
               MZ   @ @,Y2-105
     *
               D    Y2,YI-103          (1 - yrt4)/(1 + yrt4)
     *
               SBR  PRADDR,YI          PRADDR = @YI - dp
               ZA   DP,CNVOFF
               B    CNVRTN
               ...
               W
     *
     * Compute and print yterm4 = (1 + y[i])**4
     *
               CW   Y2-105,Y2-106
               CW   Y4-106
               ZA   YI,TEMP1           y[i]
     ...
     *
               ...
               W
     *
     * Compute and print a[i] = a[i-1]*yterm4
     *
               SW   TEMP2-106
               ZA   Y4,TEMP2           yterm4
               M    AI,TEMP2&107             *a[i-1]
               ZA   TEMP2&2,AI
               CW   TEMP2-106
     *
               SBR  PRADDR,AI          PRADDR = @AI - dp
               ...
               W
     *
     * Compute and print power2 = 2**(2*i + 1)
     *
               A    POW2
     ...
     *
               ...
               W
     *
     * Compute and print aterm = pow2*y[i]*(1 + y[i] + y2)
     *
               SW   TEMP2-106
               S    TEMP2-1
               MCW  @1@,TEMP2-106      (1
               ...
               SW   TEMP2-100
     *
               SBR  PRADDR,TEMP2&5     PRADDR = @TEMP2 - dp
               ZA   DP,CNVOFF
               B    CNVRTN
               MA   CNVOFF,PRADDR
               ZA   DPPRT,PRDP         PRDP = dp to print
               ...
               W
     *
     * Compute and print a[i] = a[i] - aterm
     *
               S    TEMP2&5,AI         a[i] - aterm
               CW   TEMP2-100,TEMP2-106
     *
               SBR  PRADDR,AI          PRADDR = @AI - dp
               ZA   DP,CNVOFF
               B    CNVRTN
               ...
               W
     *
     * Compute and print pi[i] = 1/a[i]
     *
               S    PI&107
               MCW  @1@,PI-103         1
               D    AI,PI-103           /pi[i]
     *
               SBR  PRADDR,PI          PRADDR = @PI - dp
               ZA   DP,CNVOFF
               B    CNVRTN
               MA   CNVOFF,PRADDR
               ZA   DPPRT,PRDP         PRDP = dp to print
     ...
     *
     LENN      DCW  1                  len(TWO)
     DP        DCW  105                actual number of decimal places (dp)
     DPPRT     DCW  100                number of decimal places to print
     ITERS     DCW  0                  iteration counter
     ITRMAX    DCW  3                  max iterations
     *
     TWO       DCW  2
               DC   #210               len = 2*dp
     SQRT2     DCW  #106               len = len(TWO) + dp
     ...
     *
     ...
     PI        DCW  #106               len = dp + 1
               DC   #107               len = dp + 2
     YRT4      DCW  #106               len = dp + 1
               DC   #2                 remainder of /2: len = 2
     POW2      DCW  0002
     *
     TEMP1     DCW  #106               len = dp + 1
     TEMP2     DCW  #211               len = len(TWO) + 2*dp
               DC   #107               remainder of /TEMP1: len = len(TEMP1) + 1
     TEMP3     DCW  0
               DC   #210               len = 2*dp
     ***************************************************************************
     *
     ...
     *
     ...
     P2APRD    DSA  000                @product (factor X factor)
